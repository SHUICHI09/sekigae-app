...
# 🛠️ ルーレット実行中のみ、プログラムから強制的にサイドバーを閉じるJavaScriptを流し込む
if 'roulette_running' in st.session_state and st.session_state.roulette_running:
    ...
    st.markdown("""
        <script>
        var sidebar = window.parent.document.querySelector('section[data-testid="stSidebar"]');
        var button = window.parent.document.querySelector('button[aria-label="Expand sidebar"]');
...
with tab_run:
    if st.session_state.final_df is None:
        ...
        if start_trigger:
            ...
            start_btn_placeholder.empty() # 実行中は開始ボタンを非表示
            st.rerun() # JavaScriptを即座に効かせてサイドバーを閉じるためにリランを入れる

    # バックグラウンドでのルーレットループ処理
    if st.session_state.roulette_running:
        active_coords = [(r, c) for r in range(7) for c in range(6) if st.session_state.seat_map[r][c]]
        df = st.session_state.final_df
        current_pool = df.head(num_students).copy()
